	...
	<div class="activity_detail_wrap" v-if="detailData">
		<y-nav :title="detailData.activityTitle" :showLeftArrow="inApp">
			<span slot="nav-right" v-if="inApp">
				<y-button type="text" class="iconfont icon-more_line" @click.native.stop="titleShare"></y-button>
			</span>
		</y-nav>
		<div class="platform_activity-content" :class="{'activity-content-noApp': !inApp}">
			...
			<p class="platform_activity-img" v-if="imgs.length">
				<img v-for="(img, index) in imgs" :src="img | imageResize(6)" :key="index" @click="picturePreview(index)">
			</p>
		...
		<div v-if="detailData" class="platform_activity-vote" :class="{'platform_activity-vote-noApp': !inApp}">
			<div class="platform_activity-vote-container" :class="{'platform_activity-vote-comment': !inApp || detailData.commentFlag !== 11}">
				<div class="tip-text" v-if="!inApp">
					...
					<div>
						马上给
						<span v-if='detailData.user' class="tip-nickname" v-text="detailData.user.userNickName"></span>
						拉票
					</div>
				</div>
				<div class="buttons">
...
export default {
	...
	methods: {
		...
		getActivityStatus() {
			...
			if (this.activityData.activityStatus === 12) {
				let start = this.activityData.activityVoteBegin;
				let end = this.activityData.activityVoteEnd;
				let current = this.activityData.currentDate;
				if (current >= end) return 'activityVoteEnd';
				if (current < start) return 'activityVoteBegin';
				return 'activityIng';
			} else if (this.activityData.activityStatus === 11)
			...
			if (resData.code === '200') {
				...
				if (this.inApp) {
					...
					if (resData.data.prizesFlag === 11) { // 有奖品
						this.$router.push({ path: `/activity/vote/${this.detailData.activityInfoId}/prizes` });
						return;
					}
					this.$router.push({ path: `/activity/vote/${this.detailData.activityInfoId}/prizesNot` }); // 无奖励页面
				}
			} 
		},	
		async titleShare() {
			...
			// 未登录：分享、收藏、举报
			// 第三方隐藏
			this.moreActionMenuList = [7, 3];
			if (this.detailData.user && this.$env.userId !== this.detailData.user.userId) {
				this.moreActionMenuList.push(5) // 举报					
			}
			this.moreActionMixinOpen();
		},
		async activityMain() {
			let params = { activityInfoId: this.detailData.activityInfoId }
	...
	async mounted() {				
		...
		this.detailData = Object.assign(this.detailData, {title: this.detailData.activityTitle})
		// 微信授权，掉接口获取活动渠道码	
